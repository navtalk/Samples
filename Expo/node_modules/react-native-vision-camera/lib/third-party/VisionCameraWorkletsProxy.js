...
//       `AsyncRunner`?
function isRuntimeThreadProvider(object) {
    if (object == null || typeof object !== 'object')
        return false;
    return (Object.hasOwn(object, 'createAsyncRunner') &&
    ...
    if (_provider == null) {
        try {
            ...
            if (!isRuntimeThreadProvider(provider)) {
                throw new Error('The module `react-native-vision-camera-worklets` does not export a `provider: RuntimeThreadProvider`!');
            }
            _provider = provider;
        }
        catch {
 ...
 */
export const VisionCameraWorkletsProxy = {
    createAsyncRunner: () => getProvider().createAsyncRunner(),
    createRuntimeForThread: (thread) => getProvider().createRuntimeForThread(thread),
    bindUIUpdatesToController: (value, controller, funcName) => getProvider().bindUIUpdatesToController(value, controller, funcName),
};
