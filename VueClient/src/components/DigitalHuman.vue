<template>
  <div class="real-time-container">
    ...
    <div class="ah-character-box">
      <div class="ah-character-avatar" style="position: relative; overflow: hidden; width: 100%; height: 100%;">
        <video id="character-avatar-video"
               v-if="isVideoPlay"
               style="position: absolute; top: 50%; left: 50%; width: 100%; height: 100%; object-fit: cover; transform: translate(-50%, -50%);"></video>
        <img v-else id="character-static-image" :src="staticImageSrc"
    ...
    <button class="ah-btn ah-btn-icon btn-character-call" id="btnRealtime" :class="{'btn-character-call-active': isCalling}" @click="toggleCall">
...
function startCall(){
  navTalkDigitalHuman.openChannel()
}
function endCall(){
  navTalkDigitalHuman.closeChannel()
}
function toggleCall(){
  if(isCalling.value){
    endCall()
...
onMounted(async () => {
  navTalkDigitalHuman.on(HumanEvent.AUDIO_WILL_PLAY,({})=>{
    isVideoPlay.value = true
  })
  navTalkDigitalHuman.on(HumanEvent.CLOSE,({})=>{
