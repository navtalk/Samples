...
class NavTalk_Avatar_Name_Tag extends \Elementor\Core\DynamicTags\Tag {
    ...
    public function render() {
        global $post;
        if (isset($post->avatar_name) || isset($post->post_title)) {
...
class NavTalk_Call_Button_Tag extends \Elementor\Core\DynamicTags\Tag {
    ...
    public function render() {
        global $post;
        if (!isset($post->avatar_id) || !$post->is_available) {
            return;
        }
        ...
        $allowed_svg_html = [
            'svg' => ['viewBox' => true, 'fill' => true, 'stroke' => true, 'stroke-width' => true, 'stroke-linecap' => true, 'stroke-linejoin' => true],
            'path' => ['d' => true],
        ];

        echo sprintf(
            '<button class="%s" data-avatar-id="%s" data-avatar-img="%s" data-modal-width="%s" data-modal-height="%s" data-modal-overlay-color="%s" data-call-button-position="%s">%s</button>',
            esc_attr($settings['button_class']),
            esc_attr($post->avatar_id),
            esc_url($image_url),
            ...
            wp_kses($phone_icon, $allowed_svg_html)
        );
    }
}
...
class NavTalk_Avatar_Status_Tag extends \Elementor\Core\DynamicTags\Tag {
    ...
    public function render() {
        ...
        if (isset($post->is_available)) {
            echo esc_html(
                $post->is_available
                    ? __('Available', 'navtalk-digital-human')
                    : __('Unavailable', 'navtalk-digital-human')
            );
        }
    }
}
